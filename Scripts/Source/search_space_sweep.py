...
Evaluates a fixed grid of baseline XGBoost configurations for a circuit,
ranks them by mean validation RMSE across a representative subset of sliding
windows, derives tighter search space bounds from the top-performing
baselines, and updates the circuit YAML file.

Usage (Linux / macOS):
    TARGET_GP_NAME="Bahrain Grand Prix" python Scripts/Source/search_space_sweep.py
    TARGET_GP_NAME="Bahrain Grand Prix" python Scripts/Source/search_space_sweep.py --dry-run
    python Scripts/Source/search_space_sweep.py --all-gps
    python Scripts/Source/search_space_sweep.py --all-gps --dry-run

Usage (Windows / PowerShell):
    $env:TARGET_GP_NAME="Bahrain Grand Prix"; python Scripts/Source/search_space_sweep.py
    $env:TARGET_GP_NAME="Bahrain Grand Prix"; python Scripts/Source/search_space_sweep.py --dry-run
...
Options:
    ...
    --max-windows N  Maximum number of sliding windows used for evaluation (default: 3).
"""
...
def _select_eval_windows(all_windows: list, max_windows: int) -> list:
    ...
    if n <= max_windows:
        return all_windows
    # Evenly spaced indices across the window list
...
def _evaluate_baseline(
    ...
    """Train and evaluate one baseline config on the given windows."""
...
def _derive_bounds(
    ...
    """
    Derive search space bounds from the top-performing baseline configs.

    For each hyperparameter the central range is [min, max] across the top
    configs. Padding is then applied according to the parameter type:
      - int: pad by int_pad on each side, enforce span >= 2.
      - float (log scale): multiply low by 1/log_factor, high by log_factor.
      - float (linear): extend by margin * half_range on each side, use a
        value-relative floor when the range is very narrow.
      - gamma: absolute ±0.5 margin, floor at 0.
    """
    bounds: dict = {}
    ...
    for param, spec in PARAM_SPECS.items():
        ...
        if spec["type"] == "int":
            ...
            # Guarantee a span of at least 2
            if high - low < 2:
                center = (low + high) // 2
...
def _print_results_table(ranked: list[dict], gp_name: str) -> None:
    print(f"\n{'=' * 72}")
    print(f"  Baseline results — {gp_name}")
    print(f"{'=' * 72}")
...
def _run_for_gp(
    ...
    window_ratio = float(config["window_ratio"])
    ...
    all_windows, _ws, _ts, _vs, _ss = build_sliding_windows(
    ...
    _print_results_table(ranked, gp_name)
    ...
    output_file = output_dir / f"{safe_name}_baseline_results.json"
    serializable = {
        "gp_name": gp_name,
        "top_n": top_n,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run baseline XGBoost configs and update YAML search space bounds."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=3,
        metavar="N",
        help="Maximum number of sliding windows used for evaluation (default: 3).",
    )
    ...
    for gp_name in gp_names:
        if gp_name is not None:
            ...
            print(
                ...
                "python Scripts/Source/search_space_sweep.py"
            )
            sys.exit(1)
