"""Driver-filtered Linear Regression with expanding-window validation and sequential holdout.

Complementary methodological baseline (NOT a replacement for model_lr_ew.py). The dataset
is filtered to a single ``Driver`` (chosen with ``--driver``) before the temporal split, so
the learning unit is closer to the per-driver trajectory logic of the LSTM Hybrid. Everything
else mirrors the main LR-EW protocol: ordering Year -> LapNumber, expanding-window validation
inside the first 80% modeling block, sequential 20% holdout, same window ratio, same metrics.

After the filter ``Driver`` is constant, so it is dropped from the categorical features
(documented in the metadata as ``dropped_constant_features``). Artifacts go to dedicated
``ew_driver`` subdirectories keyed by both circuit and driver, never overwriting the main
LR-EW artifacts. This script deliberately does NOT export hybrid baseline predictions: the
LSTM Hybrid must keep consuming the full-circuit LR-EW/XGB-EW predictions only.
"""
...
def build_lr_ew_driver_model_paths(repo_root, config, safe_driver):
    target_gp_name = str(config["target_gp_name"])
    safe_name = safe_gp_name(target_gp_name)
    model_filename = str(
        config.get("ew_driver_lr_model_filename_template", "{safe_gp_name}_{safe_driver}_lr_model_ew_driver.pkl")
    ).format(target_gp_name=target_gp_name, safe_gp_name=safe_name, safe_driver=safe_driver)
    metadata_filename = str(
        config.get(
            "ew_driver_lr_model_metadata_filename_template",
            "{safe_gp_name}_{safe_driver}_lr_model_ew_driver_metadata.json",
        )
    ).format(target_gp_name=target_gp_name, safe_gp_name=safe_name, safe_driver=safe_driver)
    subdir = str(config.get("ew_driver_lr_models_subdir", "linear_regression/ew_driver/models"))
...
def main():
    ...
    window_ratio = float(config.get("lr_ew_window_ratio", config["window_ratio"]))

    print("--- LINEAR REGRESSION (DRIVER-FILTERED): EXPANDING WINDOW + SEQUENTIAL HOLDOUT ---")
    ...
    lr_model_path, lr_model_metadata_path = build_lr_ew_driver_model_paths(repo_root, config, safe_driver)
    ...
    print(f"Saved final driver-filtered LR-EW model to: {lr_model_path}")
    print(f"Saved final driver-filtered LR-EW model metadata to: {lr_model_metadata_path}")
